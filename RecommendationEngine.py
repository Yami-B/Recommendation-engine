...
#Creating a database of users comprising id, problem solved and 
...
#Creating a table of users comprising id and other stats 
db = sqlite3.connect('users.db')
cursor = db.cursor()
cursor.execute("""
...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec  9 12:37:54 2019
...
for i in range (0,len(liste_users)):
    ...
    FROM users WHERE id="""+str(liste_users[i])+"""""")
    db.commit()
    rows = cursor.fetchall()
...
#points2=[]
#for i in range (0,len(problems)):
#    if points[i]!='' and level[i]!='':
#        level_number.append()
#        points2.append(points[i])

#Work on problem_data
level1=[]
points1=[]
tags1=[]
...
#certains users sont plus intéressés par les points, d'aures par la difficulté ou
#encore un thème
...
#None tag is significant for levels B and C so we take the tags that have the highest   
#lifts
...
def number_attempts2(User1,Prob1):
    ...
    if len(similar_problems)!=0:
       ...
       return(1)
#def number_attempts2(User1,Prob1):
#    a=Command("""SELECT AVG(attempts_range) FROM users WHERE problems="""+str(Prob1)+"""""")
#    return(a[0][0])
#reste a faire la moyenne pondérée des attempts    
User=[]
Prob_id=[]
...
#The probability to contribute above this level increases
...
#ranks_ordered=[]
#for i in range (0,len(user_id)):
#      ranks_ordered.append(Command("""SELECT rank from stats WHERE id="""+str(user_id[i])+"""""")[0][0])
#f_obs=[[0 for i in range(0,len(attempts))] for i in range (0,len(Ranks))] 
#for j in range(0,len(ranks_ordered)):
#    for k in range(0,len(attempts_range)):
#        index1=Ranks.index(ranks_ordered[j])
#        index2=attempts.index(attempts_range[k])
#        f_obs[index1][index2]+=1
#total_column=[sum(np.array(f_obs)[:,0]),sum(np.array(f_obs)[:,1]),sum(np.array(f_obs)[:,2]),
#              sum(np.array(f_obs)[:,3]),sum(np.array(f_obs)[:,4]),sum(np.array(f_obs)[:,5])]
#total_row=[sum(f_obs[0]),sum(f_obs[1]),sum(f_obs[2]),sum(f_obs[3])]
#for j in range(0,len(f_obs)):
#    for k in range(0,len(f_obs[0])):
#        f_obs[j][k]=(f_obs[j][k]-total_row[j]*total_column[k]/len(attempts_range))**2/f_obs[j][k]
#chi_deux=sum([sum(f_obs[0]),sum(f_obs[1]),sum(f_obs[2]),sum(f_obs[3])])


result=Command("""SELECT problems FROM users WHERE id="701" """)
Attempts=np.array(Command("""SELECT  attempts_range FROM users WHERE id="701" """))[:,0]

problems_of_user=[]
for i in range(0,len(result)):
    problems_of_user.append(result[i][0])
result1=[]
result2=[]
result3=[]
for i in range(0,len(problems_of_user)):
    result1.append(Command("""SELECT tags FROM problems_info WHERE problems="""+
                           str(problems_of_user[i])+""" """)[0][0])
    result2.append(Command("""SELECT points FROM problems_info WHERE problems="""+
                           str(problems_of_user[i])+""" """)[0][0])
    result3.append(int(Command("""SELECT level FROM problems_info WHERE problems="""+
                           str(problems_of_user[i])+""" """)[0][0]))
...
result32=[]
att=[]
for j in range (0,len(problems_of_user)):
    if Attempts[j]==4 or Attempts[j]==5 or Attempts[j]==6:  
        result12.append(result1[j])
        result22.append(result2[j])
        result32.append(result3[j])
        att.append(Attempts[j])
#    
#most_tried_problems=[]
#for j in range(0,len(attempts_range)):
#    if attempts_range[j]==6 and prob[j] not in most_tried_problems:
#        most_tried_problems.append(prob[j])
#most_difficult_tags=[]
#for j in range(0,len(most_tried_problems)):
#    difficult_tag=Command("""SELECT tags from problems_info WHERE problems="""+str(most_tried_problems[j])+""" """)[0][0]
#    if difficult_tag not in most_difficult_tags:
#        most_difficult_tags.append(difficult_tag)
...
def Proportion():
    proportion=[[0,0,0,0,0,0] for i in range(0,6)]
    for i in range(0,len(user_id)):
        for j in range(0,6):
            if (X1[i]==j):
                if (y[i]==1):
...
#        proportion[j]=proportion[j]/len(user_id)*100
#    a=[1,2,3,4,5,6]
#plt.plot(a,proportion)
